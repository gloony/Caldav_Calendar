...
function rcube_calendar_ui(settings)
{
    ...
    var fullcalendar_defaults = {
      ...
      eventRender: function(event, element, view) {
        ...
        // MOD by gloony - Display length of event on bar
			// var duration = moment.duration(event.end - event.start).days();
			var time = new Date(event.end - event.start - 3600000);
			var stime = '', sdays = '', shours = '', sminutes = '';
			// if(time.getDays()<10) sdays = '0';
			// sdays += time.getDays();
			if(time.getMinutes()===0){
				stime = time.getHours() + 'h';
			}else{
				if(time.getHours()<10) shours = '0';
				shours += time.getHours();
				if(time.getMinutes()<10) sminutes = '0';
				sminutes += time.getMinutes();
				stime = shours + 'h' + sminutes
			}
			if(!event.allDay){
				element.find('.fc-time').append(' <span class="fc-event-duration">[' + stime + ']</span>');
				element.find('.fc-list-item-title').prepend('<span class="fc-event-duration">[' + stime + ']</span> ');
			}
        // /MOD by gloony
      },
